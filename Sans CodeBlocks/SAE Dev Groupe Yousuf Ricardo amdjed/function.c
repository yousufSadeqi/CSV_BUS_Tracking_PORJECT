...
void modifierPassager(Trajet *t, int id, char *nouveauNom, float nouveauPrix) {
    ...
    while (i < t->nbPassagers) {
        if (t->passagers[i].id == id) {
            ...
            if (nouveauNom != NULL) {
                ...
                printf("Nom modifie : %s\n", nouveauNom);
            }

            if (nouveauPrix >= 0) {
                t->passagers[i].prix = nouveauPrix;
                printf("Prix modifie : %.2f\n", nouveauPrix);
            }
    ...
    printf("Erreur : Passager avec ID %d introuvable.\n", id);
}
...
int supprimerTrajetsDeficitaires(Trajet tab[], int *n) {
    ...
    for(int i=0; i<*n; i++) {
        float ca = chiffreAffaires(&tab[i]);
        // Un trajet est déficitaire SI CA < 0 ET ce n'est pas une erreur de calcul (N/A)
